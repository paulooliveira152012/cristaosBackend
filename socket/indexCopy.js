...
// (opcional) estado local de salas ao vivo
...
// 🔐 helper: extrai token do handshake (auth || Authorization || cookie)
function getTokenFromSocket(socket) {
  const hdrs = socket.handshake?.headers || {};
  const viaAuth = socket.handshake?.auth?.token || null;
  const viaBearer = hdrs.authorization?.startsWith("Bearer ")
    ? hdrs.authorization.split(" ")[1]
    : null;
  const viaCookie = cookie.parse(hdrs.cookie || "").token || null;
  return viaAuth || viaBearer || viaCookie || null;
}
...
function authMiddleware(io) {
  io.use((socket, next) => {
    try {
      ...
      if (!token) return next(); // conecta como guest; se quiser bloquear, use: next(new Error('Unauthorized'))
      const { id } = jwt.verify(token, process.env.JWT_SECRET);
      socket.data.userId = String(id); // guardar no socket.data
      next();
    } catch (e) {
      // em caso de token inválido/expirado, segue como guest
      // ou: next(new Error('Unauthorized'));
      next();
    }
  });
}

// 🔒 wrapper para garantir userId nos handlers
