...
router.put("/comment/like/:commentId", async (req, res) => {
  ...
  try {
    ...
    if (!listing) {
      return res.status(404).json({ message: "Listing with comment not found" });
    }
    ...
    if(!comment) {
      return res.status(404).json({ message: "Comment not found within listing" });
    }
...
router.delete("/:commentId/:parentCommentId?", async (req, res) => {
  ...
  try {
    if (!parentCommentId) {
      ...
      if (!listing) {
        return res.status(404).json({ message: "Listing with parent comment not found" });
      }
